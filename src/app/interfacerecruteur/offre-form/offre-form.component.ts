...
export class OffreFormComponent implements OnInit {
  ...
  constructor(
    ...
    this.offreForm = this.fb.group({
      specialite: ['', Validators.required],
      datePublication: [{ value: new Date().toISOString().substring(0, 10), disabled: false }],
      dateExpiration: ['', Validators.required],
      salaireMin: [0, [Validators.required, Validators.min(0)]],
      salaireMax: [0, [Validators.required, Validators.min(0)]],
      niveauExperienceRequis: ['', Validators.required],
  ...
  private patchFormValues(offre: any): void {
    // Assurez-vous que l'offre est bien l'objet attendu
    const offreData = offre.offreEmploi || offre;
    
    this.offreForm.patchValue({
      specialite: offreData.specialite,
      datePublication: new Date(offreData.datePublication).toISOString().substring(0, 10),
      dateExpiration: new Date(offreData.dateExpiration).toISOString().substring(0, 10),
      salaireMin: offreData.salaireMin,
      salaireMax: offreData.salaireMax,
      niveauExperienceRequis: offreData.niveauExperienceRequis,
      typeContrat: offreData.typeContrat,
      statut: offreData.statut,
      modeTravail: offreData.modeTravail,
      estActif: offreData.estActif,
      avantages: offreData.avantages,
      idFiliale: offreData.idFiliale,
      idDepartement: offreData.idDepartement,
      idRecruteur: offreData.idRecruteur
    });
  }
  ...
  private loadFormArrays(offre: any): void {
    ...
    const postesArray = this.postes;
    postesArray.clear();
    if (offreData.postes?.length > 0) {
      offreData.postes.forEach((poste: any) => {
        postesArray.push(this.createPosteFormGroup(poste));
      });
    } else {
      this.addPoste();
    ...
    const missionsArray = this.offreMissions;
    missionsArray.clear();
    if (offreData.offreMissions?.length > 0) {
      offreData.offreMissions.forEach((mission: any) => {
        missionsArray.push(this.createMissionFormGroup(mission));
      });
    } else {
      this.addMission();
    ...
    const languesArray = this.offreLangues;
    languesArray.clear();
    if (offreData.offreLangues?.length > 0) {
      offreData.offreLangues.forEach((langue: any) => {
        languesArray.push(this.createLangueFormGroup(langue));
      });
    } else {
      this.addLangue();
    ...
    const competencesArray = this.offreCompetences;
    competencesArray.clear();
    if (offreData.offreCompetences?.length > 0) {
      offreData.offreCompetences.forEach((competence: any) => {
        competencesArray.push(this.createCompetenceFormGroup(competence));
      });
    } else {
      this.addCompetence();
    ...
    const diplomesArray = this.diplomeIds;
    diplomesArray.clear();
    if (offreData.diplomeIds?.length > 0) {
      offreData.diplomeIds.forEach((diplomeId: string) => {
        diplomesArray.push(this.fb.control(diplomeId));
      });
    } else {
      this.addDiplome();
  ...
  private prepareSubmitData(): any {
    // Récupérer toutes les valeurs, y compris celles des contrôles désactivés
    const formValue = this.offreForm.getRawValue();
    ...
    const data = {
      idOffreEmploi: this.offerId || undefined,
      specialite: formValue.specialite,
      datePublication: this.isEditMode ? formValue.datePublication : new Date().toISOString(),
      dateExpiration: new Date(formValue.dateExpiration).toISOString(),
      salaireMin: formValue.salaireMin,
      salaireMax: formValue.salaireMax,
      niveauExperienceRequis: formValue.niveauExperienceRequis,
  ...
  private createOffre(data: any): void {
    // Pour la création, le backend attend un objet avec une propriété 'dto'
    const requestData = { dto: data };
    
    console.log('Création - Données envoyées:', requestData);
    
    this.offreService.addOffre(requestData).subscribe({
      next: (response) => {
      ...
      error: (error) => {
        ...
        this.handleError('Échec de la création', error);
      }
    });
  }

  private updateOffre(data: any): void {
    console.log('Mise à jour - Données envoyées:', data);
    
    this.offreService.update(this.offerId!, data).subscribe({
      next: (response) => {
        console.log('Réponse mise à jour:', response);
        this.handleSuccess('Offre mise à jour avec succès');
      },
      error: (error) => {
        console.error('Erreur mise à jour:', error);
        this.handleError('Échec de la mise à jour', error);
      }
    });
  }
