...
export class OffreFormComponent implements OnInit {
  ...
  constructor(
    ...
    this.offreForm = this.fb.group({
      specialite: ['', Validators.required],
      datePublication: [{ value: new Date(), disabled: !this.isEditMode }], // Added datePublication
      dateExpiration: ['', Validators.required],
      salaireMin: [0, [Validators.required, Validators.min(0)]],
      salaireMax: [0, [Validators.required, Validators.min(0)]],
      niveauExperienceRequis: ['', Validators.required],
  ...
  private patchFormValues(offre: any): void {
    ...
    this.offreForm.controls['datePublication'].disable();
  }
  ...
  createDiplomeFormGroup(diplome?: Diplome): FormGroup {
    return this.fb.group({
      nomDiplome: [diplome?.nomDiplome || '', Validators.required],
      niveau: [diplome?.niveau || '', Validators.required],
      domaine: [diplome?.domaine || '', Validators.required],
  ...
  private handleError(message: string, error: any): void {
    console.error(message, error);
    Swal.fire('Erreur', `${message}: ${error.error?.message || error.message}`, 'error');
  }
