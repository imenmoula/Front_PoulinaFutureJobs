...
export class OffreFormComponent implements OnInit {
  ...
  if (this.isEditMode) {
    ...
    // Préparer l'objet pour l'update
    const updateData = { ...formValue };
    
    Swal.fire({
      title: 'Confirmation',
      text: 'Mettre à jour l\'offre ?',
      icon: 'question',
      showCancelButton: true,
      confirmButtonText: 'Confirmer'
    }).then((result) => {
      if (result.isConfirmed) {
        this.offreEmploiService.update(id, updateData).subscribe({
          next: () => {
    ...
    // Préparer l'objet pour la création
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.offreEmploiService.addOffre(request).subscribe({
          next: () => {
