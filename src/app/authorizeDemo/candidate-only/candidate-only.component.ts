...
export class CandidateOnlyComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // Charger les offres par filiale
  ...
  loadOffresByFiliale(): void {
    this.offreEmploiService.getOffresByFiliale().subscribe({
      next: (data) => {
        console.log('Offres by filiale fetched:', data);
        this.offresByFiliale = data;
      },
      error: (err) => {
        console.error('Error fetching offres by filiale:', err);
  ...
  // Nouvelle méthode pour naviguer vers les offres d'une filiale
